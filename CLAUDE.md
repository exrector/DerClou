...
- Main tactical rendering path: **SwiftUI `RealityView` + virtual RealityKit camera + `PerspectiveCameraComponent`**, moderate field of view. **(Старый вариант: `OrthographicCameraComponent`.)** Changed by the owner on 2026-08-17 after seeing it on device: orthographic projection has no parallax, so the wall tops and the floor can only move together and the level reads as a box sliding around. The wanted effect — wall tops pinned to the display while the floor swings inside them, letting the player look into a room from different angles, as Fallout Shelter does — requires near geometry to move differently from far geometry, which only perspective gives.
- No virtual joystick.
