...
- Main tactical rendering path: **SwiftUI `RealityView` + virtual RealityKit camera + `PerspectiveCameraComponent`**, narrow field of view (26°), looking straight down and never rotating. The peek gesture is a **shear of the scene about the plane of the wall tops** (`ViewShear`), which pins the level's frame to the display; see `docs/UI_AND_CAMERA.md`. `ProjectiveTransformCameraComponent` is not usable — `RealityView` ignores it and renders black (measured 2026-08-18). **(Старый вариант: `OrthographicCameraComponent`.)** Changed by the owner on 2026-08-17 after seeing it on device: orthographic projection has no parallax, so the wall tops and the floor can only move together and the level reads as a box sliding around. The wanted effect — wall tops pinned to the display while the floor swings inside them, letting the player look into a room from different angles, as Fallout Shelter does — requires near geometry to move differently from far geometry, which only perspective gives.
- No virtual joystick.
